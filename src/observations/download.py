...
PRECIP_FIELDS = "ID%2CSTN_ID%2CUTC_DATE%2CPRECIP_AMOUNT%2CPRECIP_AMOUNT_FLAG"
...
def download_precip(args):
    hourly_range = pd.date_range(start=args.start_date, end=args.end_date, freq='1h')
    date_range = [h.tz_localize('utc') for h in hourly_range]

    for utc_date in date_range:
        print(f"⚡️ utc_date={utc_date.strftime(format="%Y-%m-%dT%H:%M:%S")}")
        download_cmd = f'wget -O {FIR_SCRATCH}/eccc_precip_data/precip_{utc_date.strftime(format="%Y-%m-%dT%H:%M:%S")}.csv "https://api.weather.gc.ca/collections/climate-hourly/items?bbox={DOMAIN_MINX},{DOMAIN_MINY},{DOMAIN_MAXX},{DOMAIN_MAXY}&UTC_DATE={utc_date.strftime(format="%Y-%m-%dT%H:%M:%S")}Z&properties={PRECIP_FIELDS}&sortby=UTC_DATE&f=csv"' # Date is specified using local time!!
        print(' > executing command: ', download_cmd)
        subprocess.run(download_cmd, shell=True, check=True)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--precip", type=bool, required=False, default=False
    )

    args = parser.parse_args()

    print(' precip : ', args.precip)
    
    if args.precip==True:
        download_precip(args)
    else:
        download(args)
