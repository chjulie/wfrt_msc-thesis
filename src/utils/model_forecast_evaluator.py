...
class ModelEvaluator(ABC):
    def __init__(self, date_range, lead_times: list[str], model_name: str, system: str):
        self.system = system
    ...
    def rmse(self, field: str):
        ...
        if field == "2t":
            ...
            observations = (
                self.current_obs_data_df[field_obs] + 273.15
            )  # convert to Kelvin
        else:
...
class DLModelEvaluator(ModelEvaluator):
    ...
    def compute_coordinates(self):
        self.coords = np.column_stack(
            (self.forecast_ds["longitude"].values, self.forecast_ds["latitude"].values)
        )
                ...
                for field_mod, field_obs in OBS_EVAL_FIELDS.items():
                    ...
                    if field_mod == "10ff":
                        try:
                            ...
                            v10 = self.forecast_ds["10v"].sel(
                                time=self.xtime(initial_date, lead_time)
                            )
                            field_data = mpcalc.wind_speed(
                                u10.values * units("m/s"), v10.values * units("m/s")
                            )
                        ...
                        try:
                            field_data = self.forecast_ds[field_mod].sel(
                                time=self.xtime(initial_date, lead_time)
                            )
                        except KeyError as e:
                            print(
