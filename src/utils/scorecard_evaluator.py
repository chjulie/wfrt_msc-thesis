...
class ScorecardEvaluator(ABC):
    def __init__(self, 
        ...
        self.scorecard_df = pd.DataFrame(columns=["initial_date", "lead_time", "field", "model", "rmse"])

        self.ground_truth_ds = xr.open_dataset(f"{FIR_SCRATCH}/data/cleaned_data/anemoi-climatex-training-6h-20230101-20231231.zarr", engine='zarr')
    ...
    def climatex_missing(self):
        return [datetime.strptime(d, '%Y-%m-%dT%H:%M:%S') for d in self.ground_truth_ds.attrs['missing_dates']]
    ...
    def save_scorecard_df(self):
        self.scorecard_df.to_csv(f"{ERROR_DATA_DIR}/scorecard-{self.model_name}-{self.date_range[0].strftime('%Y%m%d')}_{self.date_range[-1].strftime('%Y%m%d')}.csv")
                ...
                for field in self.climatex_var_map.keys():
                    ...
                    try:
                        ...
                        if ('_' in field):
                            ...
                            raw_wrf_field = self.ds[var].sel(XTIME=self.xtime(initial_date, lead_time), air_pressure=int(level)).values.flatten()
                        else: 
                            raw_wrf_field = self.ds[field].sel(XTIME=self.xtime(initial_date, lead_time)).values.flatten()
                    ...
                    if counter == 0:
                        clipped_wrf_coords, clipped_climatex_coords, wrf_mask, climatex_mask = (
